...
// A minimal setup is:
//
//	engine, err := sqlguard.NewEngine(applicationRule)
//	if err != nil {
//		return err
//	}
//
...
// instance may be called concurrently, so rules must be immutable or protect
// their own state. Context cancellation is checked before and after parsing and
// before every rule call. The synchronous CGO parser cannot be interrupted
// while its C call is in progress; cancellation is observed when parsing
// returns.
package sqlguard
